							...
							<tbody>
								...
								<c:forEach items="${pagerData.data}" var="item" varStatus="loop">
									<tr>
										...
										<td><img src="${base }/img/ProductImages/${item.path}" class="img-thumbnail w-25 h-25"></td>
										
										<td>${item.status == true ? "Active" : "Block"}</td>

										<td><a
											href="${base }/admin/post/addorupdate?id=${item.id}"
											class="btn btn-primary">Edit</a>
											<form class="d-inline">
												<button type="button"
													onclick="blockOrActivePost('${base}','${item.id}')"
