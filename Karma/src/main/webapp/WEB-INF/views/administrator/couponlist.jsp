		...
		<div id="content-wrapper" class="d-flex flex-column">
			...
			<div id="content">
				...
				<div class="container-fluid p-5">
					<form method="get" action="${base }/admin/coupons/list"
						...
						<div class="row">
							<h3 class="d-block col-9 h2">
								<select class="d-inline-block form-select w-25 col-2 "
									name="status">
									<option value="" ${searchModel.status = null ? "selected" : "" }>All</option>
									<option value="true" ${searchModel.status = true ? "selected" : "" }  >Active</option>
									<option value="false" ${searchModel.status = false ? "selected" : "" } >Block</option>
								</select><span class="h6 m-0 p-0"></span>
							</h3>
							<!-- Topbar Search -->
							<div class="col-3">
							...
							<tbody>
								...
								<c:forEach items="${pagerData.data}" var="item" varStatus="loop">
									<tr>
										...
										<td id="status">${item.status == true ? "Block" : "Active"}</td>

										<td><a class="btn btn-primary" data-bs-toggle="modal"
											data-bs-target="#myModal${i }">Detail</a> <!-- The Modal -->
											 <a href="${base }/admin/product/blockOrActive?id=${item.id}"
											type="button" class="btn btn-danger">${item.status == true ? "Block" : "Acive"}</a>
										</td>
									</tr>
								</c:forEach>
							</tbody>
