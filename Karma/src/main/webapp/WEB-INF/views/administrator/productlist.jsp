		...
		<div id="content-wrapper" class="d-flex flex-column">
			...
			<div id="content">
				...
				<div class="container-fluid p-5">
					<form method="get" action="${base }/admin/product/list"
						class=" mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search">
						<div class="row">
							<h3 class="d-block col-9 h2">
								<span class="h6 m-0 p-0">Category: </span> <select
									class="d-inline-block form-select w-25 col-2 " name="category">
									<option value="" selected>All</option>
									<c:forEach items="${category }" var="item">
										<option value="${item.id }"
											${item.id == searchModel.category ? "selected" : "" }>
											${item.name }</option>
									</c:forEach>
								</select>
							</h3>
							...
							<tbody>
								...
								<c:forEach items="${pagerData.data}" var="item" varStatus="loop">
									<tr>
										...
										<td id="status">${item.status == true ? "Active" : "Block"}</td>
										<td><img src="${base }/img/ProductImages/${item.avatar}"
											class="img-thumbnail w-25 h-25"></td>
										<td><a
											href="${base }/admin/product/addorupdate?id=${item.id}"
											class="btn btn-primary">Edit</a>
											<form class="d-inline">
												<button type="button"
													onclick="blockOrActiveProduct('${base}','${item.id}')"
													class="btn btn-danger" id="btnstatus">${item.status == true ? "Block" : "Acive"}</button>
